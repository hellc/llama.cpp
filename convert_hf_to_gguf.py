...
try:
    from mistral_common.tokens.tokenizers.base import (
        TokenizerVersion,  # pyright: ignore[reportMissingImports]
    )
    from mistral_common.tokens.tokenizers.multimodal import (  # pyright: ignore[reportMissingImports]
        DATASET_MEAN as _MISTRAL_COMMON_DATASET_MEAN,
    )
    from mistral_common.tokens.tokenizers.multimodal import (
        DATASET_STD as _MISTRAL_COMMON_DATASET_STD,
    )
    from mistral_common.tokens.tokenizers.sentencepiece import (  # pyright: ignore[reportMissingImports]
        SentencePieceTokenizer,
    )
    from mistral_common.tokens.tokenizers.tekken import (
        Tekkenizer,  # pyright: ignore[reportMissingImports]
    )
...
class ModelBase:
    ...
    def __init__(
        ...
        *,
        ...
        self.hparams = (
            ModelBase.load_hparams(self.dir_model, self.is_mistral_format)
            if hparams is None
            else hparams
        )
        ...
        if self.ftype == gguf.LlamaFileType.GUESSED:
            for _, tensor in self.get_tensors():
                ...
                if tensor.dtype == torch.bfloat16:
                    ...
                    logger.info(
                        "heuristics detected bfloat16 tensor dtype, setting --outtype bf16"
                    )
                    break
                elif tensor.dtype == torch.float16:
                    self.ftype = gguf.LlamaFileType.MOSTLY_F16
                    logger.info(
                        "heuristics detected float16 tensor dtype, setting --outtype f16"
                    )
                    break
            else:
                self.ftype = gguf.LlamaFileType.MOSTLY_F16
                logger.info(
                    "heuristics unable to detect tensor dtype, defaulting to --outtype f16"
                )
        ...
        # Mistral specific
        self.disable_mistral_community_chat_template = (
            disable_mistral_community_chat_template
        )
    ...
    def index_tensors(
        ...
        if remote_hf_model_id is not None:
            ...
            for name, remote_tensor in remote_tensors.items():
                tensors[name] = lambda r=remote_tensor: (
                    LazyTorchTensor.from_remote_tensor(r)
                )

            return tensors

        prefix = "model" if not self.is_mistral_format else "consolidated"
        part_names: list[str] = ModelBase.get_model_part_names(
            self.dir_model, prefix, ".safetensors"
        )
        is_safetensors: bool = len(part_names) > 0
        if not is_safetensors:
            part_names = ModelBase.get_model_part_names(
        ...
        if not self.is_mistral_format:
            ...
            if index_file.is_file():
                ...
                with open(index_file, "r", encoding="utf-8") as f:
                    ...
                    part_dict: dict[str, None] = dict.fromkeys(
                        weight_map.values(), None
                    )
        ...
        for part_name in part_names:
            ...
            if is_safetensors:
                ctx = cast(
                    ContextManager[Any],
                    gguf.utility.SafetensorsLocal(self.dir_model / part_name),
                )
            else:
            ...
            with ctx as model_part:
                ...
                for name in model_part.keys():
                    if is_safetensors:
                        ...
                        if self.lazy:
                            data_gen = lambda data=data: (
                                LazyTorchTensor.from_local_tensor(data)
                            )  # noqa: E731
                        else:
                            dtype = LazyTorchTensor._dtype_str_map[data.dtype]
                            data_gen = lambda data=data, dtype=dtype: (
                                torch.from_numpy(data.mmap_bytes())
                                .view(dtype)
                                .reshape(data.shape)
                            )  # noqa: E731
                    else:
                        ...
                        if self.lazy:
                            data_gen = lambda data=data_torch: (
                                LazyTorchTensor.from_eager(data)
                            )  # noqa: E731
                        else:
        ...
        if len(tensor_names_from_index) > 0:
            ...
            if (
                len(
                    tensor_names_from_parts.symmetric_difference(
                        tensor_names_from_index
                    )
                )
                > 0
            ):
                missing = sorted(
                    tensor_names_from_index.difference(tensor_names_from_parts)
                )
                extra = sorted(
                    tensor_names_from_parts.difference(tensor_names_from_index)
                )
    ...
    def dequant_model(self):
        ...
        if (quant_config := self.hparams.get("quantization_config")) and isinstance(
            quant_config, dict
        ):
            ...
            def dequant_bitnet(weight: Tensor, scale: Tensor) -> Tensor:
                ...
                shift = torch.tensor([0, 2, 4, 6], dtype=torch.uint8).reshape(
                    (4, *(1 for _ in range(len(orig_shape))))
                )
            ...
            def dequant_simple(
                weight: Tensor, scale: Tensor, block_size: Sequence[int] | None = None
            ) -> Tensor:
                scale = scale.float()
            ...
            def dequant_gptq(
                g_idx: Tensor, qweight: Tensor, qzeros: Tensor, scales: Tensor
            ) -> Tensor:
                ...
                if bits in [2, 4, 8]:
                    ...
                    wf = torch.tensor(
                        list(range(0, pack_dtype_bits, bits)), dtype=torch.int32
                    ).unsqueeze(0)
                    if self.lazy:
                        wf = LazyTorchTensor.from_eager(wf)

                    zeros = torch.bitwise_right_shift(
                        qzeros.unsqueeze(2).expand(-1, -1, pack_factor), wf.unsqueeze(0)
                    ...
                    raise NotImplementedError(
                        "3-bit gptq dequantization is not yet implemented"
                    )
            ...
            def dequant_packed(
                ...
                shifts = torch.arange(
                    0, 32 - (num_bits - 1), num_bits, dtype=torch.int32
                )
                if self.lazy:
                ...
                if zero_point is None:
                    ...
                    offset = (
                        zero_point.unsqueeze(1) >> shifts.reshape(1, -1, 1)
                    ) & mask
                ...
                unpacked = unpacked.reshape(
                    shape[0],
                    (unpacked.shape[-1] + group_size - 1) // group_size,
                    group_size,
                )
            ...
            if quant_method == "bitnet":
                for name in self.model_tensors.keys():
                    if name.endswith(".weight_scale"):
                        ...
                        self.model_tensors[weight_name] = lambda w=w, s=s: (
                            dequant_bitnet(w(), s())
                        )
                ...
                if (
                    ...
                    assert (
                        weight_config.get("group_size") is None
                    )  # didn't find a model using this yet
                    for name in self.model_tensors.keys():
                        if name.endswith(".weight_scale"):
                            weight_name = name.removesuffix("_scale")
                            w = self.model_tensors[weight_name]
                    ...
                    for name in self.model_tensors.keys():
                        if name.endswith(".weight_packed"):
                            ...
                            zero_point = self.model_tensors.get(
                                base_name + "_zero_point", lambda: None
                            )
                            new_tensors[base_name] = (
                                lambda w=w, scale=scale, shape=shape, zero_point=zero_point: (
                                    dequant_packed(
                                        w(),
                                        scale(),
                                        shape(),
                            ...
                            tensors_to_remove += [
                                base_name + n for n in ("_packed", "_shape", "_scale")
                            ]
                            if (base_name + "_zero_point") in self.model_tensors:
                                tensors_to_remove.append(base_name + "_zero_point")
                else:
                    raise NotImplementedError(
                        f"Quant format {quant_format!r} for method {quant_method!r} is not yet supported"
                    )
            else:
                raise NotImplementedError(
                    f"Quant method is not yet supported: {quant_method!r}"
                )
    ...
    def modify_tensors(
        ...
        if self.fuse_gate_up_exps and bid is not None:
            if self.match_model_tensor_name(
                new_name, gguf.MODEL_TENSOR.FFN_GATE_EXP, bid
            ):
                self._gate_exp_buffer[bid] = data_torch
            elif self.match_model_tensor_name(
                new_name, gguf.MODEL_TENSOR.FFN_UP_EXP, bid
            ):
            ...
            if bid in self._gate_exp_buffer and bid in self._up_exp_buffer:
                ...
                fused_name = self.format_tensor_name(
                    gguf.MODEL_TENSOR.FFN_GATE_UP_EXP, bid
                )
            ...
            if self.match_model_tensor_name(
                new_name, gguf.MODEL_TENSOR.FFN_GATE_EXP, bid
            ) or self.match_model_tensor_name(
                new_name, gguf.MODEL_TENSOR.FFN_UP_EXP, bid
            ):
                return []
    ...
    def prepare_tensors(self):
        ...
        if self.tensor_map.mapping:
            max_name_len = max(
                len(s) for _, s in self.tensor_map.mapping.values()
            ) + len(".weight,")
        else:
...
class TextModel(ModelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rope_parameters = (
            self.hparams.get("rope_parameters", self.hparams.get("rope_scaling")) or {}
        )
        ...
        local_rope_theta = self.find_hparam(
            [
                "local_rope_theta",
                "rope_local_theta",
                "rope_theta_local",
        ...
        if (
            "full_attention" not in self.rope_parameters
            and "sliding_attention" not in self.rope_parameters
        ):
            ...
            if (
                ...
                and (rope_type := self.rope_parameters.get("type")) is not None
            ):
    ...
    def set_gguf_parameters(self):
        ...
        if (
            n_head_kv := self.find_hparam(
                ["num_key_value_heads", "n_kv_heads"], optional=True
            )
        ) is not None:
        ...
        if (rope_type := rope_params.get("rope_type")) is not None:
            ...
            if rope_type == "linear" and rope_factor is not None:
                ...
                if (
                    yarn_ext_factor := rope_params.get("extrapolation_factor")
                ) is not None:
                    self.gguf_writer.add_rope_scaling_yarn_ext_factor(yarn_ext_factor)
                if (
                    yarn_attn_factor := rope_params.get(
                        "attention_factor", rope_params.get("attn_factor")
                    )
                ) is not None:
        ...
        if (
            local_rope_theta := self.rope_parameters.get("sliding_attention", {}).get(
                "rope_theta"
            )
        ) is not None:
        ...
        if (
            f_rms_eps := self.find_hparam(["rms_norm_eps", "norm_eps"], optional=True)
        ) is not None:
        ...
        if (
            n_experts_used := self.find_hparam(
        ...
        if (
            ...
            if score_func == "sigmoid":
                ...
                raise ValueError(
                    f"Unsupported expert score gating function value: {score_func}"
                )
    ...
    def _set_vocab_glm(self):
        ...
        special_vocab._set_special_token(
            "bos", tokenizer.get_added_vocab()["[gMASK]"]
        )  # 151331
        special_vocab._set_special_token(
        ...
        special_vocab._set_special_token(
            "eom", tokenizer.get_added_vocab()["<|observation|>"]
        )  # 151338
        special_vocab.add_to_gguf(self.gguf_writer)
    ...
    def _set_vocab_mistral(self):
        ...
        if self.is_mistral_format and local_template_file_path.is_file():
            ...
            not self.is_mistral_format
            or not self.disable_mistral_community_chat_template
        ):
            ...
            template = MistralModel.get_community_chat_template(
                vocab, template_dir, self.is_mistral_format
            )
        else:
            logger.info(
                "Not using a Mistral local or community chat template. Ensure to perform the tokenization and detokenization via `mistral-common`."
            )
            template = None
    ...
    def _set_vocab_plamo(self):
        ...
        if not tokenizer_jsonl_path.is_file():
            raise FileNotFoundError(
                f"PLaMo tokenizer file not found: {tokenizer_jsonl_path}"
            )
...
class MmprojModel(ModelBase):
    ...
    def get_audio_config(self) -> dict[str, Any] | None:
        mm_config_key = (
            "whisper_config" if "whisper_config" in self.hparams else "audio_config"
        )
    ...
    def set_gguf_parameters(self):
        ...
        if self.has_vision_encoder:
            ...
            self.gguf_writer.add_vision_embedding_length(
                self.find_vparam(["hidden_size", "vt_hidden_size"])
            )
            self.gguf_writer.add_vision_feed_forward_length(
                self.find_vparam(["intermediate_size", "vt_intermediate_size"])
            )
            self.gguf_writer.add_vision_block_count(self.find_vparam(self.n_block_keys))
            self.gguf_writer.add_vision_head_count(
                self.find_vparam(
                    ["num_attention_heads", "num_heads", "vt_num_attention_heads"]
                )
            )

            # preprocessor config
            image_mean = (
                _MISTRAL_COMMON_DATASET_MEAN
                if self.is_mistral_format
                else self.preprocessor_config["image_mean"]
            )
            image_std = (
                _MISTRAL_COMMON_DATASET_STD
                if self.is_mistral_format
                else self.preprocessor_config["image_std"]
            )
...
class LlamaModel(TextModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.hf_arch == "VLlama3ForCausalLM":
            self.hparams["num_attention_heads"] = self.hparams.get(
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        if rope_params := self.rope_parameters.get(
            "full_attention", self.rope_parameters
        ):
        ...
        if name.find("mlp.experts") != -1:
            ...
            if len(self._experts[bid]) >= n_experts * 3:
                ...
                for w_name in ["gate_proj", "up_proj", "down_proj"]:
                    ...
                    for xid in range(n_experts):
                        ename_to_retrieve = (
                            f"model.layers.{bid}.mlp.experts.{xid}.{w_name}.weight"
                        )
                    ...
                    yield from ModelBase.modify_tensors(
                        self, data_torch, merged_name, bid
                    )
...
class LlavaVisionModel(MmprojModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.hparams.get("model_type") == "pixtral":
            ...
            assert self.hparams["norm_eps"] is not None, (
                "norm_eps not found in params.json"
            )
            if self.use_break_tok:
        ...
        if hparams.get("model_type") == "pixtral":
            ...
            if "spatial_merge_size" in self.global_config:
                self.gguf_writer.add_vision_spatial_merge_size(
                    self.global_config["spatial_merge_size"]
                )
...
class SmolVLMModel(MmprojModel):
    def __init__(self, *args, **kwargs):
        ...
        if self.hparams["model_type"] == "smolvlm_vision":
            ...
            self.hparams["intermediate_size"] = self.hparams.get(
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vision_projector_scale_factor(
            self.global_config.get("scale_factor", 2)
        )
        self.gguf_writer.add_vision_use_gelu(True)

        # Add the preprocessor longest edge size
        preproc_image_size = self.preprocessor_config.get("size", {}).get(
...
class Llama4Model(LlamaModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None):
        ...
        if "gate_up_proj" in name:
            ...
            gate_proj_weight, up_proj_weight = data_torch.transpose(-1, -2).split(
                dim_half, dim=-2
            )
...
class Llama4VisionModel(MmprojModel):
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vision_projector_scale_factor(
            int(1.0 / self.hparams["pixel_shuffle_ratio"])
        )
    ...
    def modify_tensors(
        ...
        if "multi_modal_projector" in name or "vision_model" in name:
            ...
            if "multi_modal_projector.linear_1" in name:
                ...
                yield (
                    gguf.TENSOR_NAMES[gguf.MODEL_TENSOR.V_MMPROJ_FC] + ".weight",
                    data_torch,
                )
            else:
                yield from super().modify_tensors(data_torch, name, bid)
...
class Mistral3Model(LlamaModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.hparams.get("model_type") != "ministral3":
            ...
            self.tensor_map = gguf.get_tensor_name_map(
                self.model_arch, self.block_count
            )
    ...
    def set_gguf_parameters(self):
        ...
        if self.hparams.get("model_type") == "ministral3":
            ...
            assert rope_params["rope_type"] == "yarn", (
                "ministral3 rope_type must be 'yarn'"
            )
...
class GrokModel(TextModel):
    ...
    def set_vocab(self):
        ...
        if (
            not (self.dir_model / "tokenizer.json").is_file()
            or not (self.dir_model / "chat_template.jinja").is_file()
        ):
            logger.error(
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_attn_logit_softcapping(
            self.hparams.get("attn_logit_softcapping", 30.0)
        )
        self.gguf_writer.add_router_logit_softcapping(
            self.hparams.get("router_logit_softcapping", 30.0)
        )
        ...
        if self.hparams.get("rope_type") in ("yarn", "original"):
            ...
            self.gguf_writer.add_rope_scaling_orig_ctx_len(
                self.hparams["original_max_position_embeddings"]
            )
            self.gguf_writer.add_rope_scaling_yarn_ext_factor(
                self.hparams["extrapolation_factor"]
            )
            self.gguf_writer.add_rope_scaling_yarn_attn_factor(
                self.hparams["attn_factor"]
            )
        ...
        self.gguf_writer.add_attn_output_scale(
            self.hparams.get("attn_output_multiplier", rope_dim**-0.5)
        )
                ...
                if len(self._experts[bid]) >= n_experts * 3:
                    ...
                    for wid in [
                        ...
                        for xid in range(n_experts):
                            ...
                            datas.append(
                                torch.cat(tensor_list, dim=wid[2])
                        ...
                        merged_name = (
                            f"transformer.decoder_layer.{bid}.moe.{wid[0]}.weight"
                        )
...
class LLaDAModel(TextModel):
    ...
    def modify_tensors(
        ...
        n_kv_head = self.hparams.get(
            "num_key_value_heads", self.hparams.get("n_kv_heads")
        )
...
class Ernie4_5MoeModel(Ernie4_5Model):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_interleave_moe_layer_step(
            self.hparams["moe_layer_interval"]
        )
        self.gguf_writer.add_leading_dense_block_count(
            self.hparams["moe_layer_start_index"]
        )
        if (
            moe_intermediate_size := self.hparams.get("moe_intermediate_size")
        ) is not None:
            self.gguf_writer.add_expert_feed_forward_length(moe_intermediate_size)
        if (
            shared_expert_count := self.hparams.get("moe_num_shared_experts")
        ) is not None:
            ...
            if (
                ...
                is not None
                and (num_key_value_heads := self.hparams.get("num_key_value_heads"))
                is not None
            ):
...
class Qwen2VLVisionModel(MmprojModel):
    ...
    def set_gguf_parameters(self):
        ...
        if model_type == "qwen2_vl":
            ...
            assert fullatt_block_indexes is not None, (
...
class InternVisionModel(MmprojModel):
    ...
    def modify_tensors(
        ...
        if any([name.startswith(prefix) for prefix in vision_prefix]):
            ...
            if (
                ".ls" in name or ".lambda_" in name or "position_embedding" in name
            ...
            if ".qkv." in name:
                ...
                wv = data_torch[c * 2 :]
                yield from super().modify_tensors(
                    wq, name.replace("attn.qkv", "self_attn.q_proj"), bid
                )
                yield from super().modify_tensors(
                    wk, name.replace("attn.qkv", "self_attn.k_proj"), bid
                )
                yield from super().modify_tensors(
                    wv, name.replace("attn.qkv", "self_attn.v_proj"), bid
                )
            else:
                yield from super().modify_tensors(data_torch, name, bid)
...
class NemotronNanoV2VLModel(MmprojModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_clip_projector_type(
            gguf.VisionProjectorType.NEMOTRON_V2_VL
        )
    ...
    def modify_tensors(
        ...
        if "patch_generator.pos_embed" in name:
            ...
            if target_patches_per_side != max_patches_per_side:
                ...
                data_torch = data_torch.reshape(
                    1, max_patches_per_side, max_patches_per_side, n_embd
                )
                data_torch = data_torch.permute(
                    0, 3, 1, 2
                ).float()  # [1, n_embd, 128, 128]
                ...
                data_torch = data_torch.reshape(
                    1, target_patches_per_side * target_patches_per_side, n_embd
                )
        ...
        if name.startswith("vision_model.radio_model.model.") or name.startswith(
            "mlp1."
        ):
            yield from super().modify_tensors(data_torch, name, bid)
...
class Qwen2MoeModel(TextModel):
    ...
    def modify_tensors(
        ...
        if name.endswith("mlp.experts.down_proj") or name.endswith(
...
@ModelBase.register("PPLXQwen3Model")
@ModelBase.register("PPLXQwen3ContextualModel")
class PPLXQwen3Model(Qwen3Model):
    model_arch = gguf.MODEL_ARCH.QWEN3

    def set_gguf_parameters(self):
        super().set_gguf_parameters()
        # Perplexity embedding models are bidirectional (non-causal)
        self.gguf_writer.add_causal_attention(False)
        # They use MEAN pooling by default
...
class Qwen3NextModel(Qwen2MoeModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_ssm_inner_size(
            self.hparams["linear_value_head_dim"]
            * self.hparams["linear_num_value_heads"]
        )
        ...
        self.gguf_writer.add_rope_dimension_count(
            int(rope_dim * self.hparams.get("partial_rotary_factor", 0.25))
        )
    ...
    def modify_tensors(
        ...
        if "in_proj_qkvz.weight" in name:
            ...
            yield (
                self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_QKV, bid, ".weight"),
                qkv,
            )
            yield (
                self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_GATE, bid, ".weight"),
                z,
            )
        else:
            yield from super().modify_tensors(data_torch, name, bid)
...
class Qwen3VLVisionModel(MmprojModel):
    ...
    def modify_tensors(
        ...
        if name.startswith("visual.merger."):
            ...
            if suffix.startswith("linear_fc"):
                ...
                new_name = self.format_tensor_name(
                    gguf.MODEL_TENSOR.V_MMPROJ, fc_idx, suffix=f".{tail}"
                )
        ...
        if name == "visual.patch_embed.proj.weight":
            ...
            if kt != 2:
                raise ValueError(
                    "Current implementation only supports temporal_patch_size of 2"
                )
            yield (
                gguf.TENSOR_NAMES[gguf.MODEL_TENSOR.V_ENC_EMBD_PATCH] + ".weight",
                data_torch[:, :, 0, ...],
        ...
        if name == "visual.patch_embed.proj.bias":
            ...
            yield (
                gguf.TENSOR_NAMES[gguf.MODEL_TENSOR.V_ENC_EMBD_PATCH] + ".bias",
                data_torch,
            )
            return
...
class _LinearAttentionVReorderBase(Qwen3NextModel):
    ...
    def _reorder_v_heads(
        tensor: Tensor, dim: int, num_k_heads: int, num_v_per_k: int, head_dim: int
    ) -> Tensor:
        ...
        if (
            ...
            if ".in_proj_qkv." in name:
                ...
                if data_torch.ndim == 1:
                    ...
                    data_torch = self._reorder_v_heads(
                        data_torch, -1, num_k_heads, num_v_per_k, 1
                    )
...
class Phi3MiniModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_rope_freq_base(
            self.rope_parameters.get("full_attention", self.rope_parameters)[
                "rope_theta"
            ]
        )
        self.gguf_writer.add_file_type(self.ftype)
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        ...
        rope_scaling_type = rope_scaling.get(
...
class PhiMoeModel(Phi3MiniModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_expert_used_count(
            self.find_hparam(["num_experts_per_tok", "num_experts_per_token"])
        )
...
class Plamo2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_context_length(
            hparams.get("max_position_embeddings", 2048)
        )
        ...
        self.gguf_writer.add_rope_freq_base(
            self.rope_parameters.get("rope_theta", 10000)
        )
        ...
        intermediate_size = hparams.get("mamba_num_heads", 64) * hparams.get(
        ...
        self.gguf_writer.add_feed_forward_length(
            hparams.get("intermediate_size", 13312)
        )
        self.gguf_writer.add_file_type(self.ftype)
...
class Plamo3Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vocab_size(self.hparams["vocab_size"])
        if (
            sliding_window := self.find_hparam(
                ["window_size", "sliding_window"], optional=True
            )
        ) is not None:
            self.gguf_writer.add_sliding_window(sliding_window)
            self.gguf_writer.add_sliding_window_pattern(
                self.hparams["sliding_window_pattern"]
            )
...
class KimiLinearModel(TextModel):
    ...
    def set_vocab(self):
        ...
        if tokpre == "kimi-k2":
            ...
            raise NotImplementedError(
                f"Deepseek pre-tokenizer {tokpre!r} is not supported yet!"
            )
    ...
    def set_gguf_parameters(self):
        ...
        if (
            q_lora_rank := self.find_hparam(["q_lora_rank", "n_lora_q"], optional=True)
        ) is not None:
        ...
        qk_rope_head_dim = self.find_hparam(
            ["qk_rope_head_dim", "n_rot"], optional=False
        )
        ...
        if (
            n_embd_head_k_mla := self.find_hparam(["n_embd_head_k_mla"], optional=True)
        ) is not None:
        ...
        self.gguf_writer.add_expert_feed_forward_length(
            self.hparams["moe_intermediate_size"]
        )
        ...
        self.gguf_writer.add_leading_dense_block_count(
            self.hparams["first_k_dense_replace"]
        )
    ...
    def modify_tensors(
        ...
        if name.endswith("kv_b_proj.weight"):
            ...
            v_head_dim = self.find_hparam(
                ["n_embd_head_v_mla", "v_head_dim"], optional=False
            )
...
class InternLM2Model(TextModel):
    ...
    def modify_tensors(
        ...
        if bid is not None and f"model.layers.{bid}.attention.wqkv" in name:
            ...
            yield from super().modify_tensors(
                q, self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_Q, bid), bid
            )
            yield from super().modify_tensors(
                k, self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_K, bid), bid
            )
            yield from super().modify_tensors(
                v, self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_V, bid), bid
            )
        else:
            yield from super().modify_tensors(data_torch, name, bid)
...
@ModelBase.register(
    "BertModel", "BertForMaskedLM", "CamembertModel", "BertForSequenceClassification"
)
class BertModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        if self.cls_out_labels:
            self.gguf_writer.add_classifier_output_labels(
                [v for k, v in sorted(self.cls_out_labels.items())]
            )
    ...
    def _xlmroberta_set_vocab(self) -> None:
        ...
        if not tokenizer_path.is_file():
            ...
            precompiled_charsmap = b64decode(
                tokenizer_json["normalizer"]["precompiled_charsmap"]
            )
...
class NomicBertModel(BertModel):
    ...
    def __init__(
        self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, **kwargs: Any
    ):
        ...
        self.model_arch = (
            gguf.MODEL_ARCH.NOMIC_BERT_MOE
            if self.is_moe
            else gguf.MODEL_ARCH.NOMIC_BERT
        )
        ...
        npos, mtp = (
            self.hparams["n_positions"],
            self.hparams.get("max_trained_positions", 2048),
        )
        if npos == 8192 and mtp == 2048:
            self.hparams["n_positions"] = (
                2048  # nomic-embed-text v1 and v1.5 are trained for 2048 tokens.
            )
        elif npos == 2048 and mtp == 2048:
            self.hparams["n_positions"] = (
                512  # nomic-embed-text-v2-moe is trained for 512 tokens.
            )
        else:
            raise ValueError(
                f"unrecognized parameters: n_positions={npos}, max_trained_positions={mtp}"
            )

        assert (
            self.hparams["activation_function"] == "gelu" if self.is_moe else "swiglu"
        )
    ...
    def modify_tensors(
        ...
        if "mlp.experts.mlp.w1" in name:
            data_torch = data_torch.view(
                n_experts, self.hparams["n_inner"], self.hparams["n_embd"]
            )
            name += ".weight"
...
class NeoBert(BertModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_feed_forward_length(
            int(2 * self.hparams["intermediate_size"] / 3)
        )
        ...
        self.gguf_writer.add_pooling_type(
            gguf.PoolingType.CLS
        )  # https://huggingface.co/chandar-lab/NeoBERT#how-to-use
...
class XLMRobertaModel(BertModel):
    ...
    def set_gguf_parameters(self):
        ...
        if lora_prompt_prefixes := self.hparams.get("task_instructions"):
            assert self._lora_files and all(
                lora_name in lora_prompt_prefixes
                for lora_name in self._lora_files.keys()
            )
        for lora_name, lora_writer in self._lora_files.items():
            lora_writer.add_float32(
                gguf.Keys.Adapter.LORA_ALPHA,
                lora_alpha if lora_alpha is not None else 1.0,
            )
            lora_writer.add_string(gguf.Keys.Adapter.LORA_TASK_NAME, lora_name)
            if lora_prompt_prefixes:
                lora_writer.add_string(
                    gguf.Keys.Adapter.LORA_PROMPT_PREFIX,
                    lora_prompt_prefixes[lora_name],
                )
...
class Gemma3Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_context_length(
            hparams.get("max_position_embeddings", 131072)
        )
        ...
        self.gguf_writer.add_rope_freq_base(
            self.rope_parameters.get("full_attention", self.rope_parameters).get(
            ...
            if modules_file.is_file():
                ...
                for mod in mods:
                    if mod["type"] == "sentence_transformers.models.Dense":
                        ...
                        model_tensors_file = (
                            self.dir_model / mod_path / "model.safetensors"
                        )
                        if model_tensors_file.is_file():
                            ...
                            if mod_conf_file.is_file():
                                with open(
                                    mod_conf_file, encoding="utf-8"
                                ) as mod_conf_json_file:
                                    ...
                                    if (
                                        mod_conf["in_features"] is not None
    ...
    def set_gguf_parameters(self):
        ...
        with open(self.dir_model / "config.json", "r", encoding="utf-8") as f:
            ...
            if orig_sliding_window is None:
                raise ValueError(
                    "sliding_window not found in model config - this is required for the model"
                )
...
class ConformerAudioModel(MmprojModel):
    ...
    def modify_tensors(
        ...
        if "batch_norm" in name:
            ...
            weight = self._batch_norm_tensors[bid][
                f"conformer.layers.{bid}.conv.batch_norm.weight"
            ]
            bias = self._batch_norm_tensors[bid][
                f"conformer.layers.{bid}.conv.batch_norm.bias"
            ]
            running_mean = self._batch_norm_tensors[bid][
                f"conformer.layers.{bid}.conv.batch_norm.running_mean"
            ]
            running_var = self._batch_norm_tensors[bid][
                f"conformer.layers.{bid}.conv.batch_norm.running_var"
            ]
            ...
            yield from super().modify_tensors(
                a, f"conformer.layers.{bid}.conv.batch_norm.weight", bid
            )
            yield from super().modify_tensors(
                b, f"conformer.layers.{bid}.conv.batch_norm.bias", bid
            )
            return
...
class Gemma3nVisionAudioModel(ConformerAudioModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        assert self.hparams_vision is not None
        self.hparams_vision["n_layers"] = (
            128  # fake value for audio encoder, vision encoder doesn't use it
        )
        self.hparams_vision["intermediate_size"] = (
            self.hparams_vision.get("intermediate_size", 2048) * 4
        )
        self.hparams_vision["num_attention_heads"] = self.hparams_vision.get(
        ...
        self.hparams_audio["intermediate_size"] = self.hparams_audio.get(
    ...
    def set_gguf_parameters(self):
        ...
        # vision params
        self.gguf_writer.add_clip_vision_projector_type(
            gguf.VisionProjectorType.GEMMA3NV
        )
        self.gguf_writer.add_vision_attention_layernorm_eps(
            self.hparams.get("layer_norm_eps", 1e-6)
        )
    ...
    def modify_tensors(
        ...
        if new_name.endswith("conv_stem.conv.bias") or new_name.endswith(
            "layer_scale.gamma"
        ):
            data_torch = (
                data_torch.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            )  # [1, C, 1, 1]
...
class Gemma3NModel(Gemma3Model):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert self.hparams["altup_num_inputs"] == 4, (
            "Current conversion only supports 4 altup inputs"
        )
        self._altup_proj = [
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_embedding_length_per_layer_input(
            self.hparams["hidden_size_per_layer_input"]
        )
    ...
    def modify_tensors(
        ...
        if "embed_tokens.weight" in name or "embed_tokens_per_layer" in name:
            ...
            if current_size < vocab_size:
                ...
                padding = torch.zeros(
                    (padding_size, data_torch.shape[1]),
        ...
        if "altup_unembed_projections" in name:
            ...
            if out is not None:
                yield from ModelBase.modify_tensors(
                    self, out, "model.altup_unembed_projections.weight", bid
                )
                return
            else:
                return
        ...
        if "altup_projections" in name:
            ...
            if out is not None:
                yield from ModelBase.modify_tensors(
                    self, out, "model.altup_projections.weight", bid
                )
                return
            else:
                return
...
class RWKV6Qwen2Model(Rwkv6Model):
    ...
    def set_gguf_parameters(self):
        ...
        time_mix_extra_dim = self.hparams.get(
            "lora_rank_tokenshift", 64 if hidden_size >= 4096 else 32
        )
        time_decay_extra_dim = self.hparams.get(
            "lora_rank_decay", 128 if hidden_size >= 4096 else 64
        )
...
class Rwkv7Model(TextModel):
    ...
    def modify_tensors(
        ...
        if (
            "attention.v" in name
            and "value" not in self.map_tensor_name(name)
            and bid == 0
        ):
        ...
        if bid is not None and "attention.x_" in name:
            if "attention.x_x" in name:
                ...
                try:
                    ...
                    self.lerp_weights[bid] = {name: data_torch}
                if all(
                    f"model.layers.{bid}.attention.x_{i}"
                    in self.lerp_weights[bid].keys()
                    for i in lerp_list
                ):
                    new_name = f"blk.{bid}.time_mix_lerp_fused.weight"
                    data = torch.stack(
...
class Mamba2Model(TextModel):
    ...
    def modify_tensors(
        ...
        if self.match_model_tensor_name(new_name, gguf.MODEL_TENSOR.SSM_CONV1D, bid):
            ...
            data_torch = data_torch.reshape(
                (self.n_group, self.d_inner // self.n_group)
            )
...
class DeepseekV2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        moe_intermediate_size = self.find_hparam(
            ["moe_intermediate_size", "intermediate_size"], optional=False
        )
        self.gguf_writer.add_expert_feed_forward_length(moe_intermediate_size)
        ...
        if (
            norm_topk_prob := hparams.get("norm_topk_prob")
    ...
    def modify_tensors(
        ...
        if self.hparams.get("tie_word_embeddings", False):
            if name == "lm_head.weight" or name == "model.lm_head.weight":
                logger.info(
                    "Skipping tied output layer 'lm_head.weight' (will use token_embd.weight)"
                )
                return
...
class MimoV2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        assert (
            self.hparams["swa_num_attention_heads"]
            == self.hparams["num_attention_heads"]
        )
        ...
        n_head_kv_arr = [
            n_head_kv_swa if use_swa == 1 else n_head_kv
            for use_swa in self.hparams["hybrid_layer_pattern"]
        ]
        self.gguf_writer.add_head_count_kv(n_head_kv_arr)

        self.gguf_writer.add_sliding_window(self.hparams["sliding_window"])
        self.gguf_writer.add_sliding_window_pattern(
            self.hparams["hybrid_layer_pattern"]
        )
...
class Step35Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        if isinstance(rope_theta, list):
            ...
            self.rope_parameters["sliding_attention"] = {
                "rope_theta": self.hparams["local_rope_theta"]
            }
        ...
        assert [
            1.0 if lt == "sliding_attention" else 0.5 for lt in layer_types
        ] == partial_rotary_factors
        head_arr = [
            n_head_swa if lt == "sliding_attention" else n_head_base
            for lt in layer_types
        ]
        kv_arr = [
            n_kv_swa if lt == "sliding_attention" else n_kv_base for lt in layer_types
        ]
        ...
        self.gguf_writer.add_expert_shared_feed_forward_length(
            self.hparams["share_expert_dim"]
        )

        if (
            moe_router_scaling_factor := self.hparams.get("moe_router_scaling_factor")
        ) is not None:
        ...
        self.gguf_writer.add_moe_every_n_layers(
            int(self.hparams.get("moe_every_n_layer", 1))
        )
        ...
        if (limits := self.hparams.get("swiglu_limits")) is not None:
            limits_f = [
                0.0 if v is None else float(v) for v in limits[: self.block_count]
            ]
        ...
        if (limits_shared := self.hparams.get("swiglu_limits_shared")) is not None:
            limits_shared_f = [
                ...
                for v in limits_shared[: self.block_count]
            ]
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        ...
        old_context_len = int(
            rope_params.get(
                ...
                self.hparams.get("original_max_position_embeddings", 8192),
            )
        )
...
class JaisModel(TextModel):
    ...
    def modify_tensors(
        ...
        if new_name == self.format_tensor_name(gguf.MODEL_TENSOR.TOKEN_EMBD):
            yield from super().modify_tensors(
                data_torch * self.embeddings_scale, new_name, bid
            )
        elif new_name == self.format_tensor_name(gguf.MODEL_TENSOR.OUTPUT):
            yield from super().modify_tensors(
                data_torch * self.width_scale, new_name, bid
            )
        else:
            yield from super().modify_tensors(data_torch, new_name, bid)
...
class Glm4Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_rope_dimension_count(
            int(rope_dim * self.partial_rotary_factor)
        )
        ...
        if self.use_mrope:
            ...
            if name.endswith(("q_proj.weight", "q_proj.bias")):
                data_torch = Glm4Model.normal_to_neox(
                    data_torch, n_head, n_head, head_dim, self.partial_rotary_factor
                )
            if name.endswith(("k_proj.weight", "k_proj.bias")):
                data_torch = Glm4Model.normal_to_neox(
...
class GlmOCRModel(Glm4Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.block_count = self.hparams["num_hidden_layers"] + self.hparams.get(
    ...
    def set_gguf_parameters(self):
        ...
        if (
            num_nextn_predict_layers := self.hparams.get("num_nextn_predict_layers")
        ) is not None:
    ...
    def set_gguf_parameters(self):
        ...
        if hparams.get("sliding_window") is not None:
            ...
            if "layer_types" in hparams:
                self.gguf_writer.add_sliding_window_pattern(
                    [t == "sliding_attention" for t in hparams["layer_types"]]
                )
                ...
                if isinstance(hparams["sliding_window_pattern"], str):  # e.g. LLLG
                    for i in range(hparams["num_hidden_layers"]):
                        sliding_window_pattern.append(
                            hparams["sliding_window_pattern"][
                                i % len(hparams["sliding_window_pattern"])
                            ]
                            == "L"
                        )
                if isinstance(hparams["sliding_window_pattern"], int):  # e.g. 4
                    for i in range(hparams["num_hidden_layers"]):
                        sliding_window_pattern.append(
                            (i + 1) % hparams["sliding_window_pattern"] != 0
                        )
...
class ExaoneMoEModel(Exaone4Model):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_expert_weights_scale(self.hparams["routed_scaling_factor"])
        self.gguf_writer.add_expert_weights_norm(self.hparams["norm_topk_prob"])
        n_dense_layer = self.hparams.get(
            "first_k_dense_replace", self.hparams.get("first_last_k_dense_replace", 0)
        )
        self.gguf_writer.add_leading_dense_block_count(n_dense_layer)
        self.gguf_writer.add_nextn_predict_layers(
    ...
    def modify_tensors(
        ...
        if name.startswith("mtp."):
            if name.find("layers.") != -1:
                ...
                name = name.replace(
                    f"mtp.layers.{bid}",
                    f"model.layers.{bid + self.hparams['num_hidden_layers']}",
                )
            else:
                # mtp fc/norm weights
                remapper = {
                ...
                for bid in range(self.hparams["num_hidden_layers"], self.block_count):
                    yield from super().modify_tensors(
                        data_torch, new_name.format(bid=bid), bid
                    )
                return
...
class GraniteMoeModel(GraniteModel):
    ...
    def set_gguf_parameters(self):
        ...
        if shared_feed_forward_length := self.hparams.get("shared_intermediate_size"):
            self.gguf_writer.add_expert_shared_feed_forward_length(
                shared_feed_forward_length
            )
            logger.info(
                "gguf: (granitemoeshared) shared_feed_forward_length = %s",
                shared_feed_forward_length,
            )
    ...
    def modify_tensors(
        ...
        if name.endswith("shared_mlp.input_linear.weight"):
            ...
            assert data_torch.shape[-2] == 2 * ffn_dim, (
                "Merged FFN tensor size must be 2 * shared_intermediate_size"
            )
            gate, up = data_torch.split(ffn_dim, dim=-2)
            if has_experts:
                yield from ModelBase.modify_tensors(
            ...
            yield from ModelBase.modify_tensors(
                self, up, self.format_tensor_name(gguf.MODEL_TENSOR.FFN_UP, bid), bid
            )
            return
        ...
        if not has_experts and name.endswith("shared_mlp.output_linear.weight"):
            yield from ModelBase.modify_tensors(
                self,
                data_torch,
                self.format_tensor_name(gguf.MODEL_TENSOR.FFN_DOWN, bid),
                bid,
            )
            return
...
class GraniteHybridModel(Mamba2Model, GraniteMoeModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._ssm_layers = [
            i for i in range(self.block_count) if i not in self._attn_layers
        ]
        ...
        if not self._ssm_layers:
            ...
            new_arch = (
                gguf.MODEL_ARCH.GRANITE_MOE if has_experts else gguf.MODEL_ARCH.GRANITE
            )
        ...
        self.d_model = self.find_hparam(
            [f"{self.hparam_prefixes[0]}_head_dim", "hidden_size", "d_model"]
        )
    ...
    def get_attn_layers(self):
        ...
        if not attn_layers:
            ...
            assert attn_offset is not None, (
                "No attention layer offset set with attn_layer_period"
            )
            attn_layers = [
                i for i in range(self.block_count) if i % attn_period == attn_offset
            ]
        return attn_layers
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_ssm_conv_kernel(
            self.find_hparam(["conv_kernel", "d_conv"])
        )
        self.gguf_writer.add_ssm_state_size(
            self.find_hparam(["state_size", "d_state", "state_dim", "ssm_state_size"])
        )
        self.gguf_writer.add_ssm_group_count(self.n_group)
        self.gguf_writer.add_ssm_inner_size(self.d_inner)
        ...
        self.gguf_writer.add_ssm_time_step_rank(
            self.find_hparam(["n_heads", "num_heads"])
        )

        ## Attention params ##
        head_count_kv = self.find_hparam(["num_key_value_heads", "n_head_kv"])
        head_count_kv_vec = [
            head_count_kv if i in self._attn_layers else 0
            for i in range(self.block_count)
        ]
        ...
        use_rope = (
            "BambaForCausalLM" in self.hparams["architectures"] or not self._ssm_layers
        )
        ...
        assert self.hparams.get("hidden_act") in [None, "silu"], (
            "Only SILU activation supported"
        )
        assert self.d_inner % d_head == 0, (
            f"SSM inner size {self.d_inner} not a multiple of head dim {d_head}"
        )
...
class NemotronHModel(GraniteHybridModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.head_dim = self.hparams.get(
            "head_dim", self.hparams.get("attention_head_dim")
        )
        assert self.head_dim is not None, (
            "Could not find the attention head dim in config"
        )
        ...
        self._ssm_layers = [
            i for i, val in enumerate(hybrid_override_pattern) if val == "M"
        ]
        self._mlp_layers = [
            i
            for i, val in enumerate(hybrid_override_pattern)
            if val == ("E" if self.is_moe else "-")
        ]

    def get_attn_layers(self):
        hybrid_override_pattern = self.hparams["hybrid_override_pattern"]
        assert len(hybrid_override_pattern) == self.block_count, (
            "Mismatch between hybrid override and num_hidden_layers!"
        )
    ...
    def set_gguf_parameters(self):
        ...
        if not self.is_moe:
            ...
            self.gguf_writer.add_feed_forward_length(
                [
                    moe_intermediate_size if i in self._mlp_layers else 0
                    for i in range(self.block_count)
                ]
            )
            ...
            self.gguf_writer.add_expert_shared_feed_forward_length(
                self.hparams["moe_shared_expert_intermediate_size"]
            )
            self.gguf_writer.add_expert_count(self.hparams["n_routed_experts"])
            self.gguf_writer.add_expert_shared_count(self.hparams["n_shared_experts"])
            self.gguf_writer.add_expert_weights_norm(self.hparams["norm_topk_prob"])
            self.gguf_writer.add_expert_weights_scale(
                self.hparams["routed_scaling_factor"]
            )
...
class BailingMoeModel(TextModel):
    ...
    def modify_tensors(
        ...
        if name.endswith("attention.dense.weight"):
            yield from super().modify_tensors(
                data_torch,
                self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_OUT, bid),
                bid,
            )
            return
        elif name.endswith("query_key_value.weight"):
            q, k, v = data_torch.split(
                [n_head * head_dim, n_kv_head * head_dim, n_kv_head * head_dim], dim=-2
            )

            yield from super().modify_tensors(
                BailingMoeModel.permute(q, n_head, n_head),
                self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_Q, bid),
                bid,
            )
            yield from super().modify_tensors(
                BailingMoeModel.permute(k, n_head, n_kv_head),
                self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_K, bid),
                bid,
            )
            yield from super().modify_tensors(
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_expert_shared_feed_forward_length(
            hparams.get(
                ...
                hparams["moe_intermediate_size"] * hparams["num_shared_experts"],
            )
        )
...
class GroveMoeModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_expert_chunk_feed_forward_length(
            self.hparams.get("head_dim") or 128
        )
    ...
    def modify_tensors(
        ...
        if name.find("chunk_experts") != -1:
            n_experts = (
                self.find_hparam(["num_local_experts", "num_experts"]) // 2
            )  # see add_experts_per_group
            assert bid is not None
            ...
            if len(self._chunk_experts[bid]) >= n_experts * 3:
                ...
                for w_name in ["down_proj", "gate_proj", "up_proj"]:
                    ...
                    merged_name = (
                        f"model.layers.{bid}.mlp.chunk_experts.{w_name}.weight"
                    )
...
class GlmASRWhisperEncoderModel(MmprojModel):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if (
            "hidden_size" not in self.hparams
            and "intermediate_size" not in self.hparams
        ):
            self.hparams["hidden_size"] = self.hparams["d_model"]
            self.hparams["intermediate_size"] = self.hparams["encoder_ffn_dim"]
            self.hparams["num_attention_heads"] = self.hparams[
                "encoder_attention_heads"
            ]
    ...
    def modify_tensors(
        ...
        if name.startswith("audio_encoder.adapting."):
            name = name.replace(
                "audio_encoder.adapting.", "audio.multi_modal_projector."
            )
...
class VoxtralWhisperEncoderModel(WhisperEncoderModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_audio_stack_factor(
            4
...
class AudioFlamingo3WhisperEncoderModel(WhisperEncoderModel):
    def set_gguf_parameters(self):
        super().set_gguf_parameters()
        self.gguf_writer.add_clip_projector_type(
            gguf.VisionProjectorType.MUSIC_FLAMINGO
        )
...
class FalconH1Model(Mamba2Model):
    ...
    def modify_tensors(
        ...
        if "down_proj" in name:
            ...
            groups_time_state_size = (
                self.hparams["mamba_n_groups"] * self.hparams["mamba_d_state"]
            )
            tensor[:intermediate_size, :] *= zxbcdt_multipliers[0]
            tensor[intermediate_size : 2 * intermediate_size, :] *= zxbcdt_multipliers[
                1
            ]
            tensor[
            ...
            tensor[
                2 * intermediate_size + groups_time_state_size : 2 * intermediate_size
                + 2 * groups_time_state_size,
                :,
            ] *= zxbcdt_multipliers[3]
            tensor[2 * intermediate_size + 2 * groups_time_state_size :, :] *= (
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_block_count(self.block_count)
        self.gguf_writer.add_context_length(
            self.hparams.get("max_position_embeddings", 0)
        )
        self.gguf_writer.add_feed_forward_length(self.hparams["intermediate_size"])
...
class HunYuanMoEModel(TextModel):
    ...
    def set_vocab(self):
        ...
        special_tokens = tokenizer.special_tokens
        reverse_vocab = {
            id_: encoded_tok for encoded_tok, id_ in {**vocab, **special_tokens}.items()
        }
        tokens: list[str] = []
        toktypes: list[int] = []
        for i in range(vocab_size):
    ...
    def set_gguf_parameters(self):
        ...
        if self.rope_parameters.get("rope_type") == "dynamic":
            ...
            scaled_base = base * (
                alpha ** (dim / (dim - 2))
            )  # 10000 * (1000 ** (128 / 126)) = 11158839.9251
...
class LLaDAMoEModel(TextModel):
    ...
    def set_gguf_parameters(self):
        super().set_gguf_parameters()
        if (
            expert_intermediate_size := self.hparams.get("expert_intermediate_size")
        ) is not None:
        ...
        if (self.dir_model / "tokenizer.json").is_file():
            ...
            if self.hparams["hidden_size"] == 4096:
                self.gguf_writer.add_bos_token_id(
                    127958
                )  # only for 7b dense, fix <|bos|> token
...
class GptOssModel(TextModel):
    ...
    def repack_mxfp4(self, new_name: str, blocks: Tensor, scales: Tensor):
        ...
        logger.info(
            f"Repacked {new_name} with shape {new_shape} and quantization MXFP4"
        )
        # flatten last dim
        new_data = new_data.view(
            new_data.shape[0], new_data.shape[1], new_data.shape[2] * new_data.shape[3]
        )
        new_data = new_data.numpy()
        self.gguf_writer.add_tensor(
            new_name, new_data, raw_dtype=gguf.GGMLQuantizationType.MXFP4
        )
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        ...
        for name, data_torch in self.get_tensors():
            if "mlp.experts.down_proj_blocks" in name:
                ...
                new_name_gate = self.map_tensor_name(
                    name.replace("gate_up_proj_scales", "gate_proj.weight")
                )
                new_name_up = self.map_tensor_name(
                    name.replace("gate_up_proj_scales", "up_proj.weight")
                )
    ...
    def modify_tensors(
        ...
        if "experts" in name:
            ...
            for w_name in expert_weights:
                ...
                for xid in range(n_experts):
                    ename = (
                        f"model.layers.{bid}.feed_forward.experts.{xid}.{w_name}.weight"
                    )
                    datas.append(expert_cache[ename])
                    del expert_cache[ename]
...
class LFM2VLModel(MmprojModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vision_attention_layernorm_eps(
            self.find_vparam(["layer_norm_eps"])
        )
        self.gguf_writer.add_vision_projector_scale_factor(
            self.global_config.get("downsample_factor", 2)
        )
        self.gguf_writer.add_vision_use_gelu(True)
        # python notation, e.g. for vision_feature_layer == -1, we pick last layer -> vision_feature_layers_to_drop = 0
        vision_feature_layers_to_drop = -(
            self.global_config.get("vision_feature_layer", -1) + 1
        )
        self.gguf_writer.add_vision_block_count(
            self.find_vparam(self.n_block_keys) - vision_feature_layers_to_drop
        )
    ...
    def modify_tensors(
        ...
        if is_vision_tensor:
            ...
            if "patch_embedding.weight" in name:
                data_torch = data_torch.view(data_torch.shape[0], 16, 16, 3).permute(
                    0, 3, 1, 2
                )
...
class SmallThinkerModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        if (n_experts := self.hparams.get("moe_num_primary_experts")) is not None:
            self.gguf_writer.add_expert_count(n_experts)
        if (
            n_experts_used := self.hparams.get("moe_num_active_primary_experts")
        ) is not None:
            self.gguf_writer.add_expert_used_count(n_experts_used)
        if (
...
class MistralModel(LlamaModel):
    ...
    @staticmethod
    def get_community_chat_template(
        vocab: MistralVocab, templates_dir: Path, is_mistral_format: bool
    ):
        assert (
            TokenizerVersion is not None
            and Tekkenizer is not None
            and SentencePieceTokenizer is not None
        ), _mistral_import_error_msg
        ...
        if vocab.tokenizer.version == TokenizerVersion.v1:
            ...
            vocab.tokenizer.version == TokenizerVersion.v3
            and vocab.tokenizer_type == MistralTokenizerType.spm
        ):
            return "mistral-v3"
        elif (
            vocab.tokenizer.version == TokenizerVersion.v3
            and vocab.tokenizer_type == MistralTokenizerType.tekken
        ):
            return "mistral-v3-tekken"
        elif (
            vocab.tokenizer.version == TokenizerVersion.v7
            and vocab.tokenizer_type == MistralTokenizerType.spm
        ):
            return "mistral-v7"
        elif (
    ...
    def set_mistral_config(gguf_writer: gguf.GGUFWriter, hparams: dict):
        if "yarn" in hparams:
            ...
            gguf_writer.add_rope_scaling_orig_ctx_len(
...
class MistralMoeModel(DeepseekV2Model):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_attn_temperature_length(
            yarn_params["original_max_position_embeddings"]
        )
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None):
        if (
            ...
            or name.startswith("patch_merger.")
            or "mm_projector" in name
        ):
            return
    ...
    def modify_tensors(
        ...
        if is_vision_tensor:
            if "pos_emb.weight" in name:
                data_torch = data_torch.view(
                    data_torch.shape[0] * data_torch.shape[1], data_torch.shape[2]
                )
...
class KimiK25Model(MmprojModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert self.hparams_vision is not None, (
            "Kimi-K2.5 requires vision_config in model config"
        )

        self.merge_kernel_size = tuple(
            self.hparams_vision.get("merge_kernel_size", [2, 2])
        )
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_uint32(
            "vision.pos_emb_height", self.hparams_vision.get("init_pos_emb_height", 64)
        )
        self.gguf_writer.add_uint32(
            "vision.pos_emb_width", self.hparams_vision.get("init_pos_emb_width", 64)
        )
        self.gguf_writer.add_uint32(
            "vision.pos_emb_time", self.hparams_vision.get("init_pos_emb_time", 4)
        )

        # Projector parameters
        self.gguf_writer.add_vision_use_gelu(
            self.hparams_vision.get("projector_hidden_act", "gelu") == "gelu"
        )
        self.gguf_writer.add_vision_attention_layernorm_eps(
    ...
    def modify_tensors(
        ...
        if "wqkv" in name:
            ...
            if "weight" in name:
                wq, wk, wv = (
                    data_torch[:qkv_dim, :],
                    data_torch[qkv_dim : 2 * qkv_dim, :],
                    data_torch[2 * qkv_dim :, :],
                )
                ...
                bq = (
                    bq.reshape(n_head, head_dim // 4, 2, 2)
                    .permute(0, 2, 1, 3)
                    .reshape(-1)
                )
                bk = (
                    bk.reshape(n_head, head_dim // 4, 2, 2)
                    .permute(0, 2, 1, 3)
                    .reshape(-1)
                )
...
class JanusProVisionModel(MmprojModel):
    def __init__(self, *args, **kwargs):
        ...
        if "intermediate_size" not in self.hparams_vision:
            ...
            if mlp_ratio is not None and hidden_size is not None:
                self.hparams_vision["intermediate_size"] = int(
                    round(hidden_size * mlp_ratio)
                )
    ...
    def _map_aligner_tensor(
        ...
        tensor_name = self.format_tensor_name(
            gguf.MODEL_TENSOR.V_MMPROJ, mm_index, suffix=suffix
        )
...
class YoutuVLVisionModel(MmprojModel):
    ...
    def set_gguf_parameters(self):
        ...
        if hidden_act in (
            ...
            raise ValueError(
                f"Unsupported activation function for YOUTUVL: {hidden_act}"
            )

        self.gguf_writer.add_vision_spatial_merge_size(
            self.hparams.get("spatial_merge_size", 2)
        )
        ...
        assert fullatt_block_indexes is not None, (
            "fullatt_block_indexes is required for youtuvl"
        )
...
class LazyTorchTensor(gguf.LazyBase):
    ...
    def from_safetensors_slice(cls, st_slice: Any) -> Tensor:
        ...
        lazy = cls(
            ...
            args=(st_slice,),
            func=lambda s: s[...] if len(s.get_shape()) == 0 else s[:],
        )
    ...
    def from_local_tensor(cls, t: gguf.utility.LocalTensor) -> Tensor:
        def load_tensor(tensor: gguf.utility.LocalTensor) -> Tensor:
            ...
            return (
                torch.from_numpy(byteswap_tensor(tensor.mmap_bytes(), numpy_dtype))
                .view(dtype)
    ...
    def from_remote_tensor(cls, remote_tensor: gguf.utility.RemoteTensor):
        ...
        lazy = cls(
            ...
            func=lambda r: (
                torch.from_numpy(
                    byteswap_tensor(
                        np.frombuffer(r.data(), dtype=numpy_dtype), numpy_dtype
                    )
                )
                .view(dtype)
                .reshape(shape)
...
def main() -> None:
    ...
    if args.remote:
        ...
        local_dir = snapshot_download(
            repo_id=hf_repo_id, allow_patterns=allowed_patterns
        )
    ...
    with torch.inference_mode():
        ...
        if not is_mistral_format:
            ...
            assert hparams.get("vision_encoder") is not None, (
